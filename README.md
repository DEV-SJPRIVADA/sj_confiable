...
| `GET/POST` crear y `PUT` editar solicitud | **Crear:** 1–5 servicios **o** un paquete (excluyentes), como `procesarSolicitud.php`. **Editar:** solo 1–5 servicios (`actualizarSolicitud.php`); sin campo paquete; `reemplazarServicios` en pivote; `paquete_id` no se modifica en edición. UI: desplegables con casillas (`_picklist-checkbox`, `picklist-checkbox.css` / `picklist-checkbox.js`) — cerrados muestran *Seleccione servicio* / *Seleccione paquete*; exclusión mutua en alta. Edición si activa y no Completada/Cancelada; historial y aviso a consultores SJ al guardar. |
...
- **Selección servicios/paquete (UI):** partial `resources/views/panel/partials/_picklist-checkbox.blade.php`; estilos `public/css/legacy/picklist-checkbox.css`; script `public/js/solicitudes/picklist-checkbox.js` (validación cliente, máx. 5 servicios, un paquete, mutex crear).
